...
import os
import lpips
from diffusers import StableDiffusionControlNetPipeline, ControlNetModel, UNet2DConditionModel, AutoencoderKL, UNet2DConditionModel, UVit2DModel, Transformer2DModel
from networks.decoder import ImageDecoder
...
class ViTTrainer(BaseTrainer):
    def __init__(self, args):
        ...
        self.vit = torchvision.models.vision_transformer.VisionTransformer(
			image_size=512,
			patch_size=8,
			num_layers=12,
			num_heads=12,
        ...
        self.decoder = ImageDecoder(latent_dim=768, out_channels=3)
        
        self.diffuser_optim = torch.optim.AdamW(
            list(self.vit.parameters()) + list(self.decoder.parameters()), lr=args.ns_lr)
        ...
        self.vit, self.decoder, self.diffuser_optim, self.diffuser_scheduler = self.accelerator.prepare(
            self.vit, self.decoder, self.diffuser_optim, self.diffuser_scheduler)
    ...
    def train_one_step(self, batch):
        ...
        with self.accelerator.autocast():
            recons_feats = self.vit(condition_tensor).reshape(-1, 64, 64, 768).permute(0, 3, 1, 2)
            recons_img = self.decoder(recons_feats)
    ...
    def validation(self, eval_datachunk, step):
        ...
        for ci, chunk in enumerate(eval_datachunk):
            ...
            for bi, batch in enumerate(dataloader):
                ...
                with self.accelerator.autocast():
                    recons_feats = self.vit(condition_tensor).reshape(-1, 64, 64, 768).permute(0, 3, 1, 2).contiguous()
                    image = self.decoder(recons_feats)
                    image = (image + 1.0) / 2.0
